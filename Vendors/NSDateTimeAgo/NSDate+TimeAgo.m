...
#warning EDIT
    //Was getting yesterday for today, must be time now problem.
    NSTimeZone *tz = [NSTimeZone defaultTimeZone];
    NSInteger seconds = [tz secondsFromGMTForDate:now];
    now = [NSDate dateWithTimeInterval: seconds sinceDate:now];
    ...
    NSDateComponents *components = [calendar components:NSCalendarUnitHour
                                               fromDate:self
                                                ...
                                                options:0];
#warning was >= 6
    if (components.hour >= 0) // if more than 6 hours ago, change precision
    {
